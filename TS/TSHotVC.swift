...
class TSHotVC: TSBaseVC ,UITableViewDataSource,UITableViewDelegate {
    ...
    func getData() {
        guard let data:Array<Dictionary<String,Any>> = UserDefaults.standard.object(forKey: "TSMainHotData") as? Array<Dictionary<String, Any>> else {
            let hud:UIActivityIndicatorView = TSTool.loading()
            TSNet.post(url: "source/sources", param: ["categoryName":"tangshiqimeng23"]) {[self] (resp) in
                print(resp)
                let data:Array<Dictionary<String,Any>> = resp["data"] as! Array<Dictionary<String,Any>>
                UserDefaults.standard.set(data, forKey: "TSMainHotData")
